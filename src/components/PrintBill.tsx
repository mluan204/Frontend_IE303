...
export async function printBillToPDF(bill: Bill) {
  ...
  container.innerHTML = `
    ...
    <p style="text-align: center;">Mã hóa đơn ${bill.id}</p>
    ...
    <table style="width:100%; border-collapse:collapse; margin-top:20px" border="1">
      <thead>
        <tr style="border-top: 1; border-bottom: 1">
          <th>Sản phẩm</th>
          <th>SL</th>
          <th>Đơn giá</th>
          <th>Giảm</th>
      ...
      <tbody>
        ${bill.billDetails
          ...
          <tr>
            ...
            <td>${item.afterDiscount.toLocaleString()}</td>
            <td>${((item.afterDiscount?item.afterDiscount:item.price) * item.quantity).toLocaleString()}</td>
          </tr>`
          )
          .join("")}
      </tbody>
    ...
    <p>+ ${bill.notes}</p>
    <p style="text-align:center;margin-top:30px"><i>Cảm ơn quý khách đã mua hàng tại JDK Store!</i></p>
  `;
